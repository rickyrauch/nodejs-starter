...
var log = require('debug')('platform:user');
...
app.get('/user/search', restrict, function (req, res) {
  ...
  api.user.search(q, function(err, users) {
    ...
    res.json(users.map(expose('id fullName gravatar() profilePictureUrl notifications')));
  });
});
...
app.get('/user/:id', restrict, function (req, res) {
  ...
  api.user.get(req.params.id, function (err, user) {
    ...
    res.json(expose('id fullName gravatar() profilePictureUrl notifications')(user));
  });
});
