...
var log = require('debug')('platform:db-api:citizen');
 ...
 * @return {Citizen}
 */
exports.create = function create(citizenData, fn) {
  var citizen = new Citizen(citizenData);
  citizen.save(function (err) {
    if (!err) {
      log('Saved citizen %s', citizen.id);
      return fn(null, citizen);
    }

    if (11000 == err.code) {
      log('Attempt Citizen duplication');
    } else {
 ...
 * @return {Citizen} a `citizen` module
 * @api public
 */
